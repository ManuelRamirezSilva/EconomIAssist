...
- 🧠 **IA Avanzada**: Powered by Azure OpenAI GPT-4.1.
...
EconomIAssist incluye una integración con WhatsApp para interactuar con el agente conversacional directamente desde tu dispositivo móvil. Sigue estos pasos para configurarlo:

1. **Configurar el entorno**:
   - Asegúrate de tener configurado el archivo `.env` con las credenciales necesarias para WhatsApp.
   - Revisa el archivo `whatsapp-simple/.env.example` para obtener un ejemplo de configuración.
...
3. **Escanear el código QR**:
   - Una vez que el servidor esté en ejecución, abre WhatsApp en tu dispositivo móvil.
   - Escanea el código QR que aparece en la terminal para vincular tu cuenta.

4. **Interactuar con el agente**:
   - Envía mensajes al número vinculado para realizar consultas financieras, registrar gastos, o recibir consejos.
...
---
## ⚠️ Importante: Uso de Branches

Este repositorio contiene múltiples ramas de desarrollo. Por favor, ten en cuenta lo siguiente:

- **No accedas a las ramas de desarrollo**: Las ramas distintas de `main` están destinadas a pruebas y desarrollo activo. Pueden contener código incompleto o inestable.
- **Versión funcional**: La versión completamente funcional y estable del proyecto se encuentra en la rama `main`.
