...
# System prompt for intent counting
...
# Prompt to split a message into its distinct intents
split_intents_prompt = (
    "You are an expert at analyzing user messages for a financial assistant. "
...
#     "Sing me a song.",
#     "I got payed today and I spent 200 pesos on groceries.",
#     "Add a meeting for tomorrow and tell me my account balance.",
#     "I want to talk to a human and also add dentist appointment on Friday at 10am.",
#     "me sobran 200 pe que me rocomendas?? debo algo??"
# ]
...
for example in examples:
    ...
    count_response = client.chat.completions.create(
        messages=[
            {"role": "system", "content": count_intents_prompt},
    ...
    if num_intents == 1:
        ...
        try:
            ...
            if not isinstance(intent_texts, list) or not all(isinstance(x, str) for x in intent_texts):
                intent_texts = [example] * num_intents
        except Exception:
            intent_texts = [example] * num_intents
